...
- (void)LunarForSolarYear:(CalendarDayModel *)calendarDay{
    ...
    [comps setYear:calendarDay.year];
    [comps setMonth:calendarDay.month];
    [comps setDay:calendarDay.day];
    
    NSCalendar *calendar = [NSCalendar calendarWithIdentifier:NSCalendarIdentifierGregorian];
    NSDate *date =[calendar dateFromComponents:comps];
    calendar = [NSCalendar calendarWithIdentifier:NSCalendarIdentifierChinese];
    comps = [calendar components:(NSCalendarUnitMonth|NSCalendarUnitDay) fromDate:date];
    NSArray *cDayName =  [NSArray arrayWithObjects:@"*",@"初一",@"初二",@"初三",@"初四",@"初五",@"初六",@"初七",@"初八",@"初九",@"初十",
                          @"十一",@"十二",@"十三",@"十四",@"十五",@"十六",@"十七",@"十八",@"十九",@"二十",
                          @"廿一",@"廿二",@"廿三",@"廿四",@"廿五",@"廿六",@"廿七",@"廿八",@"廿九",@"三十",nil];
    NSArray *cMonName =  [NSArray arrayWithObjects:@"*",@"正月",@"二月",@"三月",@"四月",@"五月",@"六月",@"七月",@"八月",@"九月",@"十月",@"十一月",@"十二月",nil];
    
    if(comps.day != 1)
        calendarDay.Chinese_calendar = [cDayName objectAtIndex:comps.day];
    else
        calendarDay.Chinese_calendar = [cMonName objectAtIndex:comps.month];
