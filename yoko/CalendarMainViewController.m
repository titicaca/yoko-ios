...
NSTimeInterval secondsPerDays = 86400;
...
- (void)viewDidLoad {
  ...
  //  NSLog(@"%@",[self.chineseDays objectAtIndex:(self.myDateComponents.day)]);
    ...
    [self.CollectionViewOfMonthCalendar scrollToItemAtIndexPath:indexPath atScrollPosition:UICollectionViewScrollPositionNone animated:NO];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
   ...
   // self.dateIterator = [NSDate dateWithTimeIntervalSince1970:(indexPath.row-7) * secondsPerDay * (indexPath.section + 1)];
    ...
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:indexPath.row*secondsPerDays - 4*secondsPerDays];
    
    self.myDateComponents = [self.myCalendar components:calendarUnit fromDate:date];
    
    
    
    cell.LabelOfDay.text = [NSString stringWithFormat:@"%li",self.myDateComponents.day];
    
    self.myDateComponents = [self.myLunarCalendar components:calendarUnit fromDate:date];
...
- (void)initCollectionView{
    ...
    [self.CollectionViewOfMonthCalendar setPagingEnabled:YES];
}
